...
Отвечает на комментарии в группах, используя Hugging Face для генерации ответов.
Если HF недоступен, использует Agnes или шаблоны.
"""
...
IGNORE_USER_IDS = [317272476]  # ваш ID и ID ботов, если они комментируют
...
def generate_huggingface_response(comment_text):
    ...
    if not HUGGINGFACE_API_KEY:
        return None
    try:
        # Используем модель для диалогов (или другую подходящую)
        model = "microsoft/DialoGPT-medium"  # можно заменить на "google/flan-t5-base"
        ...
        if resp.status_code == 200:
            ...
            if isinstance(result, list) and len(result) > 0:
                ...
                # Обрезаем, чтобы оставить только ответ (часто модель повторяет ввод)
...
# ---------- ЗАПУСК ----------
def main():
    logger.info("🚀 Бот-комментатор запущен (с Hugging Face)")
