...
# -*- coding: utf-8 -*-
"""
Бот-комментатор для ВКонтакте (финальная рабочая версия).
Использует прямой вызов методов API через vk.wall.createComment.
"""
...
# Группы для мониторинга (без музыкальной)
GROUPS_TO_WATCH = [
...
def process_comment(vk, group_id, post_id, comment_id, user_id, comment_text):
    ...
    try:
        # Используем прямой вызов метода API через vk.wall.createComment
...
def listen_group(vk, group_id):
    ...
    while True:
        try:
            ...
            data = resp.json()
            if data.get('updates'):
                logger.info(f"📦 Получено {len(data['updates'])} обновлений для группы {group_id}")
...
# ---------- ЗАПУСК ----------
def main():
    logger.info("🚀 Бот-комментатор запущен (финальная версия с прямым вызовом API)")
