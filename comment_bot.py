...
# -*- coding: utf-8 -*-
"""
Бот-комментатор для ВКонтакте (с расширенным логированием).
"""
...
def get_longpoll_server(vk, group_id):
    try:
        resp = vk.groups.getLongPollServer(group_id=abs(group_id), lp_version=3)
        logger.info(f"🔑 Получен Long Poll сервер для группы {group_id}: {resp}")
...
def process_comment(vk, group_id, post_id, comment_id, user_id, comment_text):
    if user_id in IGNORE_USER_IDS:
        logger.info(f"Игнорируем комментарий от {user_id}")
        return
...
def listen_group(vk, group_id):
    ...
    logger.info(f"👂 Начинаем слушать группу {group_id} (server={server})")
    ...
    while True:
        try:
            ...
            if resp.status_code != 200:
                logger.warning(f"Ошибка Long Poll {group_id}: статус {resp.status_code}, ответ: {resp.text}")
                time.sleep(5)
                continue

            data = resp.json()
            # Логируем ВЕСЬ ответ для отладки
            logger.info(f"📦 Long Poll ответ для группы {group_id}: {json.dumps(data, ensure_ascii=False)[:500]}")
            ...
            if updates:
                logger.info(f"📦 Получено {len(updates)} обновлений для группы {group_id}")
                for update in updates:
                    logger.info(f"  -> update: {update}")  # логируем каждое обновление
                    if update.get('type') == 'wall_reply_new':
                        obj = update.get('object', {})
                        comment = obj.get('comment') if 'comment' in obj else obj
...
# ---------- ЗАПУСК ----------
def main():
    logger.info("🚀 Бот-комментатор запущен (расширенное логирование)")
