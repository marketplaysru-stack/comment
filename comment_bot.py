...
# -*- coding: utf-8 -*-
"""
Бот-комментатор для ВКонтакте (диагностическая версия).
Использует groups.getLongPollServer.
...
def process_comment(vk, group_id, post_id, comment_id, user_id, comment_text):
    if user_id in IGNORE_USER_IDS:
        logger.debug(f"Игнорируем комментарий от {user_id}")
        return
    ...
    if state.get("last_processed", {}).get(key):
        logger.debug(f"Комментарий {key} уже обработан")
        return
        ...
        try:
            ...
            for update in updates:
                if update.get('type') == 'wall_reply_new':
                    ...
                    # В некоторых версиях API комментарий находится напрямую в object
...
# ---------- ЗАПУСК ----------
def main():
    logger.info("🚀 Бот-комментатор запущен (диагностическая версия)")
